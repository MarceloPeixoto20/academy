...
export default function Indicacoes() {
  ...
  const [campanhaSelecionadaId, setCampanhaSelecionadaId] = useState("");
  ...
  const campanhaSelecionada = useMemo(() => campanhas.find((c) => c.id === campanhaSelecionadaId) || campanhas.find((c) => c.status === "ATIVA") || campanhas[0], [campanhas, campanhaSelecionadaId]);
  const linksAlunos = useMemo(() => campanhaSelecionada ? alunos.map((aluno) => ({ id: aluno.id, nome: aluno.nome, tipo: "ALUNO", link: `${window.location.origin}/indicar/${campanhaSelecionada.id}/ALUNO:${aluno.id}:${campanhaSelecionada.id}` })) : [], [alunos, campanhaSelecionada]);
  const linksColaboradores = useMemo(() => campanhaSelecionada ? colaboradores.map((colaborador) => ({ id: colaborador.id, nome: colaborador.nome, tipo: "COLABORADOR", link: `${window.location.origin}/indicar/${campanhaSelecionada.id}/COLABORADOR:${colaborador.id}:${campanhaSelecionada.id}` })) : [], [colaboradores, campanhaSelecionada]);
  ...
  async function load() {
    try {
      ...
      if (!campanhaSelecionadaId && campanhasResp.length) setCampanhaSelecionadaId(campanhasResp.find((c) => c.status === "ATIVA")?.id || campanhasResp[0].id);
  ...
  return <section>
    <div className="page-title row"><div><h1>Indicações</h1><p>Indicações manuais, campanhas e links automáticos por aluno/colaborador.</p></div>{activeTab === "indicacoes" && <button onClick={() => setShowManualForm(true)}>Cadastrar indicação manual</button>}</div>
    {error && <div className="alert">{error}</div>}{success && <div className="success">{success}</div>}

    <div className="tabs clean-tabs"><button className={activeTab === "indicacoes" ? "tab active" : "tab"} onClick={() => setActiveTab("indicacoes")}>Indicações</button><button className={activeTab === "campanhas" ? "tab active" : "tab"} onClick={() => setActiveTab("campanhas")}>Campanhas e links</button></div>

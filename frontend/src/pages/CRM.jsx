...
export default function CRM() {
  ...
  return (
    <section>
      <div className="page-title row"><div><h1>CRM</h1><p>Dashboard comercial, últimas oportunidades e Kanban em aba separada.</p></div><div className="row-actions"><button className="ghost" onClick={() => setShowEtapas(!showEtapas)}>Configurar etapas</button><button onClick={nova}>Nova oportunidade</button></div></div>
      {error && <div className="alert">{error}</div>}{success && <div className="success">{success}</div>}
      <div className="tabs clean-tabs"><button className={activeTab === "resumo" ? "tab active" : "tab"} onClick={() => setActiveTab("resumo")}>Dashboard e lista</button><button className={activeTab === "kanban" ? "tab active" : "tab"} onClick={() => setActiveTab("kanban")}>Kanban</button></div>
